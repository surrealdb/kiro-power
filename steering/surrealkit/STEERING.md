...
A steering file for driving [SurrealKit](https://github.com/surrealdb/surrealkit), SurrealDB's schema-management and migration CLI.
...
This steering covers the SurrealKit tool itself. To write the actual schema, seed, and query statements that go in `.surql` files, see the `surrealql` steering.
...
- Store SurrealQL in files with the `.surql` extension. Validate and format generated SurrealQL with the tools described in the `surrealql` steering (`surreal validate`, `npx @surrealdb/surql-fmt`).
