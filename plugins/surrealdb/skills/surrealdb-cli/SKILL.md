...
`surreal validate` (SurrealQL file checking) and formatting are covered by the **surrealql** skill. Newer subcommands (`mcp`, `module`) also exist — run `surreal help` and `surreal <command> --help` to confirm the surface of an installed version. To connect through an instance's built-in MCP route, use the **surrealdb-mcp** skill.
